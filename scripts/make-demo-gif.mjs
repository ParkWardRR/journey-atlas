...
//
//   node scripts/make-demo-gif.mjs [trips/01-norway-kystriksveien.yaml]
//
...
const SRC = process.argv[2] || 'trips/01-norway-kystriksveien.yaml';
const NAME = 'trips/my-trip.yaml';        // friendly name shown in the demo
const STYLE = process.env.STYLE || 'opentopo';
...
hist.push(out('# 4 · screenshot the map to a print-ready PNG', 'note'));
...
const DUR = [0.6, 2.8, 0.5, 1.8, 0.5, 1.9, 0.5, 1.7, 0.8, 2.2, 2.2];

// ── page template ────────────────────────────────────────────────────────────
const page = (f) => `<!doctype html><html><head><meta charset="utf8"><style>
  * { margin: 0; box-sizing: border-box; }
  html,body { width: 1000px; height: 700px; background: #0b0e14; }
  .win { width: 1000px; height: 700px; background: #14171f; display: flex; flex-direction: column;
  ...
  .body { flex: 1; padding: 20px 24px; color: #cdd9e5; font-size: 16.5px; line-height: 1.62; overflow: hidden; }
